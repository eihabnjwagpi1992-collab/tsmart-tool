...
ADMIN_ID = os.getenv("ADMIN_ID", "7175591691")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if str(user_id) != str(ADMIN_ID):
        await update.message.reply_text(
            "⛔ عذراً، هذا البوت خاص بمدير أداة TSP TOOL PRO فقط."
        )
        return
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                "🔑 توليد مفتاح (سنة كاملة)", callback_query_data="gen_12"
            )
        ],
        [
            InlineKeyboardButton(
                "👥 عرض المستخدمين النشطين", callback_query_data="view_users"
            )
        ],
