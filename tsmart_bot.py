...
def generate_pro_key(length=12):
    return 'TSMART-PRO-' + ''.join(random.choices(string.ascii_uppercase + string.digits, k=length))
...
def handle_generate_pro_key(message):
    ...
    bot.reply_to(message, f"✅ *New 1-Year Pro License Generated:*\n`{new_key}`\n\n_This key will activate the MTK Pro and Xiaomi modules._", parse_mode="Markdown")
